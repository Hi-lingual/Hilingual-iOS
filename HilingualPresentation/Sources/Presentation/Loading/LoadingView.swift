...
final class LoadingView: BaseUIView {
    ...
    func configure(for state: State) {
        ...
        case .loading:
            ...
            subtitleLabel.text = subtitleMessages.first
    ...
    private func startSubtitleRotation() {
        ...
        subtitleTimer = Timer.scheduledTimer(withTimeInterval: 3.0, repeats: true) { [weak self] _ in
            ...
            DispatchQueue.main.async {
                self.currentSubtitleIndex = (self.currentSubtitleIndex + 1) % self.subtitleMessages.count
                UIView.transition(with: self.subtitleLabel,
                                  duration: 0.3,
                                  options: [.transitionFlipFromTop],
                                  animations: {
                                      self.subtitleLabel.text = self.subtitleMessages[self.currentSubtitleIndex]
                                  },
                                  completion: nil)
            }
        }
