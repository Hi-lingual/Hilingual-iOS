...
final class AnchoredBannerAdView: UIView {
    ...
    private func makeAnchoredAdaptiveAdSize(width: CGFloat) -> GADAdSize? {
        let current = GADCurrentOrientationAnchoredAdaptiveBannerAdSizeWithWidth(width)
        if IsGADAdSizeValid(current), current.size.width > 0, current.size.height >= 50 {
            return current
        }

        let portrait = GADPortraitAnchoredAdaptiveBannerAdSizeWithWidth(width)
        if IsGADAdSizeValid(portrait), portrait.size.width > 0, portrait.size.height >= 50 {
            return portrait
        }
        return nil
    }

    private func makeOrReuseBannerView(adSize: GADAdSize, rootViewController: UIViewController) -> GAMBannerView {
        if let bannerView {
...
private final class BannerDelegateProxy: NSObject, GADBannerViewDelegate {
